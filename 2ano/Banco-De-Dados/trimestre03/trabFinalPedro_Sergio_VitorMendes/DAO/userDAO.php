...
require_once dirname(__FILE__) . '/funcionarioDAO.php';
...
class UserDAO
{
    ...
    {
        $pdo = Database::getInstance()->getPdo();
        $sql = "SELECT u.*, l.* FROM usuario u JOIN login l on l.id = u.id WHERE u.id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($user['acesso'] == 'funcionario') {
            $funcionarioDAO = new funcionarioDAO();
            $funcionario = $funcionarioDAO->getFuncionario($funcionarioDAO->getIDbyUserID($id));
            $user['salario'] = $funcionario['salario'];
            $user['setor'] = $funcionario['setor'];
        }
