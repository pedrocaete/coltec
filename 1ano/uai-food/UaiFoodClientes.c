...
Restaurante *ler_arq_rests(FILE **arq_rests, int *tam) {
  ...
  while (fread(&aux, sizeof(Restaurante), 1, (*arq_rests))) {
    (*tam)++;
    restaurantes =
        (Restaurante *)realloc(restaurantes, (*tam) * sizeof(Restaurante));
...
int escrever_arq_clientes(FILE *arq_clientes, Cliente *clientes,
                          int total_clientes) {
...
int escrever_arq_pedidos(FILE *arq_pedidos, Pedidos *pedidos,
                         int total_pedidos) {
...
void cadastrocliente(Cliente **clientes, int *total_clientes) {
  ...
  do {
    ...
    (*total_clientes)++;
    *clientes =
        (Cliente *)realloc(*clientes, (*total_clientes) * sizeof(Cliente));
    ...
    (*clientes)[(*total_clientes) - 1]
        .nome[strlen((*clientes)[(*total_clientes) - 1].nome) - 1] = '\0';
...
void Resumo_Pedido(Pedidos *pedidos, Cliente *clientes, int num_pedidos,
                   int cod) {
  int entrega = 0, pagamento;
  float preco_total = 0;
  char formatacao[151];
  char ultimoRestaurante[MAX_DESCRICAO_PRATO];
  int restaurantes_usados[9999] = {0};  // Vetor para controlar restaurantes que já tiveram taxa de entrega cobrada
  ...
  for (int i = 0; i < num_pedidos; i++) {
    if (pedidos[i].codigo == cod) {
      // Verifica se a taxa de entrega já foi aplicada para o restaurante atual
      int taxa_aplicada = 0;
      for (int j = 0; j < i; j++) {
        if (strcmp(pedidos[i].restaurante, pedidos[j].restaurante) == 0 && restaurantes_usados[j] == 1) {
          taxa_aplicada = 1;  // Se já foi aplicada, marca como duplicado
          break;
        }
      }

      // Se a taxa ainda não foi aplicada, aumenta a taxa de entrega
      if (!taxa_aplicada) {
        printf("\n%s\n", pedidos[i].restaurante);
        entrega += 1;  // Aumenta a taxa de entrega
      }

      // Exibe o item do pedido
      printf("%dx %s = %.2f\n", pedidos[i].qtd, pedidos[i].nome,
             pedidos[i].qtd * pedidos[i].preco);
      preco_total += pedidos[i].qtd * pedidos[i].preco;

      // Marca o restaurante como já utilizado para a taxa de entrega
      for (int k = 0; k < num_pedidos; k++) {
        if (strcmp(pedidos[k].restaurante, pedidos[i].restaurante) == 0) {
          restaurantes_usados[k] = 1;  // Marca o restaurante para taxa de entrega
        }
      }
    }
  }
  ...
  sprintf(formatacao, "Taxa de entrega x%d = %.2f\nTOTAL DO PEDIDO = %.2f",
          entrega, (float)entrega * 4.99, preco_total + (float)entrega * 4.99);
  interface(formatacao);
  ...
  for (int j = 0; j < total_pedidos; j++) {
    ...
    if (pedidos[j].codigo == cod) {
      ...
      for (int i = 0; i < total_pedidos; i++) {
        ...
        if ((i == 0 ||
             strcmp(pedidos[i].restaurante, pedidos[i - 1].restaurante) != 0) &&
            pedidos[i].codigo == cod) {
          printf("\n%s\n", pedidos[i].restaurante);
          entrega += 1;
          vf = 1;
        }
...
void pedidosuaifood(Cliente *clientes, Restaurante *restaurantes, Prato *pratos,
                    Pedidos **pedidos, int quantidadeRestaurantes,
                    int total_clientes, int total_pratos, int *total_pedidos) {
    ...
    if (x == -1)
      ...
      case 3:
        if ((*total_pedidos) > 1)
          ...
          maiorcod = 1;
        // Para colocar códigos consecutivos nos clientes, começando a partir de
        // 1
        do {
          printf("Bem-vindo(a) %s - Selecione o restaurante\n",
                 clientes[x].nome);
          for (int i = 0; i < quantidadeRestaurantes; i++) {
            if (restaurantes[i].codigo > 0) {
          ...
          if (opcao > 0) {
            if (arrayrestaurante == -1)
              ...
              for (int i = 0; i < total_pratos; i++)
                if (restaurantes[arrayrestaurante].codigo ==
                    pratos[i].restaurante) {
                  printf("%d - %s - Preço: R$ %.2f\n", pratos[i].codigo,
                         pratos[i].descricao, pratos[i].preco);
                }
              printf("0 - Fechar Pedido\n-1 - Voltar ao menu anterior\n");
              scanf("%d", &prato_escolhido);
              limpaBuffer();
              for (int i = 0; i < total_pratos; i++) {
                if (prato_escolhido == pratos[i].codigo &&
                    restaurantes[arrayrestaurante].codigo ==
                        pratos[i].restaurante) {
                  arrayprato = i;
                  break;
                }
              ...
              if (prato_escolhido > 0) {
                if (arrayprato == -1)
                  ...
                  printf("%s R$%.2f. Qual a quantidade? ",
                         pratos[arrayprato].descricao,
                         pratos[arrayprato].preco);
                  scanf("%d", &qtd);
                  limpaBuffer();
                  if (qtd > 0) {
                    (*total_pedidos)++;
                    (*pedidos) = (Pedidos *)realloc(
                        *pedidos, (*total_pedidos) * sizeof(Pedidos));
                    if ((*pedidos) == NULL) {
                      perror("Erro na realocação de memória para pedidos.\n");
                      return;
                    }
                    (*pedidos)[(*total_pedidos) - 1].qtd = qtd;
                    (*pedidos)[(*total_pedidos) - 1].preco =
                        pratos[arrayprato].preco;
                    strcpy((*pedidos)[(*total_pedidos) - 1].restaurante,
                           restaurantes[arrayrestaurante].nome);
                    strcpy((*pedidos)[(*total_pedidos) - 1].nome,
                           pratos[arrayprato].descricao);
...
int main() {
  ...
  int total_rests = 0, total_pratos = 0, total_clientes = 0, total_pedidos = 0,
      opcao;
  ...
  do {
    ...
    case 1:
      ...
      break;
    case 2:
      pedidosuaifood(clientes, restaurantes, pratos, &pedidos, total_rests,
                     total_clientes, total_pratos, &total_pedidos);
      break;
    case -1:
      break;
    default:
